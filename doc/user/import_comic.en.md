...
Synchronization performs bounded recursive discovery for unmarked nested libraries. A directory with ordinary root images is a comic, and a directory whose direct children all use chapter-like names such as `Chapter 01` or `Volume 2` is treated as the comic root. A directory containing `metadata.json` remains the strongest boundary. Add that marker to ambiguous layouts so categories or chapters cannot be mistaken for comics.
